...
const checkProximity = async () => {
  try {
    // Only check notifications that haven't been matched yet
    const result = await db.query(
      `SELECT 
       ...
       WHERE an.expires_at > CURRENT_TIMESTAMP
         AND an.proximity_matched = false`
    );
    ...
    for (const notification of result.rows) {
      try {
        ...
        if (distance <= PROXIMITY_THRESHOLD) {
          console.log(`✅ Proximity match! Ride ${notification.ride_id} - Distance: ${distance.toFixed(2)}m`);

          // Update ride status to 'pending' (waiting for both parties to confirm)
          await db.query(
            `UPDATE rides SET status = 'pending' WHERE id = $1`,
            [notification.ride_id]
          );

          // Mark notification as proximity matched
          await db.query(
            `UPDATE active_notifications SET proximity_matched = true WHERE id = $1`,
            [notification.notification_id]
          );

          // Don't delete notification yet - keep it until both confirm
          // We'll delete it after confirmation or expiry
    ...
    if (expiredResult.rows.length > 0) {
      ...
      for (const expired of expiredResult.rows) {
        // Only cancel if still in 'notified' status (not yet matched)
        await db.query(
